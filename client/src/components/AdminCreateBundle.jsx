...
export default function AdminCreateBundle() {
	...
	// Fetch ALL approved courses (from all teachers)
	useEffect(() => {
		const fetchApprovedCourses = async () => {
			try {
				const token = localStorage.getItem("token");
				const response = await axios.get(
					"http://localhost:5000/api/admin/courses/approved",
					{
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			await axios.post(
				"http://localhost:5000/api/admin/bundles",
				{
			...
			setSuccess("Bundle created and published successfully!");
			...
			<div className="max-w-6xl mx-auto">
				...
				<div className="text-center mb-12">
					...
					<p className="text-lg text-gray-600">
						Select 3 approved courses from any teacher to create a bundle
					</p>
					<p className="text-sm text-orange-600 mt-2">
						Admin bundles are published immediately
					</p>
				</div>
							...
							<form onSubmit={handleSubmit} className="space-y-4">
								...
								<button
									...
									className={`w-full py-3 rounded-lg font-semibold transition-colors ${
										selectedCourses.length === 3 && !loading
											? "bg-orange-500 text-white hover:bg-orange-600"
											: "bg-gray-300 text-gray-500 cursor-not-allowed"
									}`}>
									{loading ? "Creating..." : "Create & Publish Bundle"}
								</button>

								<p className="text-xs text-green-600 text-center font-semibold">
									✓ Bundle will be published immediately
								</p>
							</form>
						</div>
					</div>
